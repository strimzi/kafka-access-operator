...
In addition to that, you can configure other variables that are used in the release pipeline.
For customizing them, click on `Variables` section under all the parameters mentioned above.
You can configure:

- `CENTRAL_USERNAME` - username - or token name - of your user token that can be generated here https://central.sonatype.com/usertoken (when logged in)
- `CENTRAL_PASSWORD` - the actual token generated here https://central.sonatype.com/usertoken (when logged in)
- `GPG_SIGNING_KEY` - GPG key for signing the artifacts
- `GPG_PASSPHRASE` - passphrase for the GPG key configured under `GPG_SIGNING_KEY`

---
#### Generating GPG key for signing the artifacts

When we are publishing the artifacts, we are also signing them using GPG key.
In the pipelines, we have default GPG keys generated by one of the maintainers, but if you want to have your own, you can do following:

1. Generate the GPG key - `gpg --full-generate-key` - or you can use the key that you are using for signing commits in GitHub for example
2. It will ask you for password, this password is important - you will use it for `GPG_PASSPHRASE` variable
3. Using `gpg --list-keys` take your key ID and store it for next step
4. Once you have the key generated and you have the ID, you have to publish it to the public keyserver
   ```shell
   gpg --keyserver keyserver.ubuntu.com --send-keys <YOUR_KEY_ID>
   ```
   This step is IMPORTANT. Without it, the release pipeline will fail on signing the artifacts.
5. Finally, you need to export the key, encode it using Base64 - then you can copy the key and use it for `GPG_SIGNING_KEY`
   ```shell
   gpg --export-secret-keys <YOUR_KEY_ID> | base64 > private.key
   ```
   The contents of `private.key` needs to be copied and then pasted into the `GPG_SIGNING_KEY`
---
